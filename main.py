...
from scraper import get_clean_calendar_data, VALID_WEEKS
...
def get_calendar(
    ...
    """
    Query params:
    - week: "this" (default), "next", or "last"
    ...
    if week not in VALID_WEEKS:
        ...
        return {
            "success": False,
            "error": f"Invalid 'week' value '{week}'. Must be one of: {sorted(VALID_WEEKS)}"
        }

    try:
        calendar_data, was_cached = get_clean_calendar_data(week=week)
